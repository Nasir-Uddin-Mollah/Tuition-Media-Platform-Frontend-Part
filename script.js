const handleRegistration = (event) => {
    ...
    const confirm_password = document.getElementById('registration-confirm-password').value;
    const info = {
    ...
    if (password === confirm_password) {
        ...
        if (/^(?=.*\d)(?=(.*\W){2})(?=.*[a-zA-Z])(?!.*\s).{1,15}$/.test(password)) {
            fetch('https://tuition-lagbe.onrender.com/user/register/', {
                ...
                .then((data) => {
                    console.log(data);
                    document.getElementById('success-messages').innerText = "Registration successful! Please check your email to activate your account.";
            ...
            document.getElementById('error-messages').innerText = "password must contain at least 1 digit 2 special characters & 1 alphabetic character";
        }
    }
    else {
        document.getElementById('error-messages').innerText = "password and confirm password doesn't match.";
    }
...
const loadTuitions = (search) => {
    ...
    fetch(`https://tuition-lagbe.onrender.com/tuition/list/?search=${search ? search : ''}`)
        .then(res => res.json())
        .then(data => {
            if (data.length > 0) {
...
const loadTuitionDetails = () => {
    ...
    fetch(`https://tuition-lagbe.onrender.com/application/review/list/?tuition_id=${id}`)
        .then(res => res.json())
        .then(data => {
                ...
                data.forEach(review => {
                    ...
                    div.innerHTML = `
                        <div class="card-body">
                            ...
                            <p class="">By <strong>${review.user_details.username}</strong> on ${new Date(review.created_at).toLocaleString()}</p>
                        </div>
                    `;
    ...
    if (token) {
        fetch(`https://tuition-lagbe.onrender.com/application/list/?user_id=${user_id}`, {
            method: 'GET',
            headers: {
                Authorization: `Token ${token}`,
                'Content-Type': 'application/json',
            ...
            .then(data => {
                // console.log(data);
                const accepted = data.some(tuition => tuition.tuition === id && tuition.status === 'accepted');
...
const displayTuitionDetails = (tuition) => {
    ...
    if (token) {
        fetch(`https://tuition-lagbe.onrender.com/application/list/?user_id=${user_id}`, {
            ...
            .then(data => {
                console.log(data);
                const application = data.find(tuition => tuition.tuition === tuition_id);
...
const handleApply = (event) => {
    ...
    fetch(`https://tuition-lagbe.onrender.com/application/list/`, {
        ...
        .catch(error => {
            console.error('Error:', error);
            alert(error.message || 'An error occurred while applying. Please try again.');
        });
};
...
const handleChangePassword = (event) => {
    ...
    if (new_password === confirm_password) {
        ...
        if (/^(?=.*\d)(?=(.*\W){2})(?=.*[a-zA-Z])(?!.*\s).{1,15}$/.test(new_password)) {
            fetch('https://tuition-lagbe.onrender.com/user/change_password/', {
                ...
                .then(data => {
                    ...
                    console.log(data);
                    document.getElementById('change-pass-success-messages').innerText = "Password changed successfully!";
                ...
                .catch((error) => {
                    ...
                    alert(error.message || 'An error occurred. Please try again.');
                    document.getElementById('change-pass-error-messages').innerText = error.message || 'An error occurred. Please try again.';
                })
        }
        else {
            document.getElementById('change-pass-error-messages').innerText = "password must contain at least 1 digit 2 special characters & 1 alphabetic character";
        }
    }
    else {
        document.getElementById('change-pass-error-messages').innerText = "new password and confirm new password doesn't match.";
    }
}
